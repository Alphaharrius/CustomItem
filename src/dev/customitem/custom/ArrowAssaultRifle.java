...
public class ArrowAssaultRifle extends CustomItemHandler {

    public static final String DISPLAY_NAME = "Arrow Assault Rifle";
    ...
    public void handlesPlayerLeftClickInteractEvent(PlayerInteractEvent event) {
        ...
        Runnable coolDownCallbackRunnable = () -> world.playSound(player.getLocation(), Sound.BLOCK_PISTON_EXTEND, 0.8f, 0.8f);
        BukkitTask coolDownTask = this.startCountdown(customItemId, 3, coolDownCallbackRunnable);
        if (coolDownTask == null) { return; }

        world.playSound(player.getLocation(), Sound.ENTITY_DRAGON_FIREBALL_EXPLODE, 0.5f, 1.5f);
        world.playEffect(player.getLocation().add(0.0f, 1.0f, 0.0f), Effect.SMOKE, 5);
        Arrow arrow = player.launchProjectile(Arrow.class);
        arrow.setTicksLived(100);
        arrow.setPickupStatus(AbstractArrow.PickupStatus.DISALLOWED);
        float randomOffset = (float) (0.1 * Math.exp(0.8 * player.getVelocity().length()));
        Vector randomVector = new Vector(Math.random() - 0.5, Math.random() - 0.5, Math.random() - 0.5).normalize().multiply(0.01 + randomOffset);
        arrow.setVelocity(arrow.getVelocity().add(randomVector).multiply(1.5));
    ...
    public void handlesPlayerRightClickInteractEvent(PlayerInteractEvent event) {
        ...
        ItemConsumer itemConsumer = new ItemConsumer(player.getInventory(), Material.ARROW, 30, Material.GUNPOWDER, 30);
        ...
        LoreReader.setProperty(loreList, CAPACITY_KEY, 30);
        LoreReader.setLoreForItemStack(itemStack, loreList);
    }
